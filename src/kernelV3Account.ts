import { KernelV3Account } from "generated";

const TYPE_VALIDATOR = "0x01";
const TYPE_PERMISSION = "0x02";

KernelV3Account.RootValidatorUpdated.handlerWithLoader({
  loader: ({ event, context }) => {
    const account = event.srcAddress.toLowerCase();
    return context.KernelV3Account.get(`${event.chainId}-${account}`);
  },
  handler: async ({ event, context, loaderReturn: account }) => {
...
KernelV3Account.ModuleInstalled.handlerWithLoader({
  loader: ({ event, context }) => {
    const account = event.srcAddress.toLowerCase();
    return context.KernelV3Account.get(`${event.chainId}-${account}`);
  },
...
KernelV3Account.ModuleUninstalled.handlerWithLoader({
  loader: ({ event, context }) => {
    const account = event.srcAddress.toLowerCase();
    return context.KernelV3Account.get(`${event.chainId}-${account}`);
  },
